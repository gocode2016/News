...
public class SearchUtil {
	...
	public void index(LuceneData data) throws Exception {
		id = data.getId();
		title = data.getTitle();
		...
		try {
			...
			/**
			 * 在Lucene4.x中，只能给域加权，部门给文档加权，如果要提高文档的加权，需要给 文档的每个域进行加权
			 * **/
	...
	public void index(LuceneData data, IKAnalyzer analyzer) throws Exception {
		id = data.getId();
		title = data.getTitle();
		content = data.getContent();
		introduction = data.getIntroduction();
		...
		try {
			...
			for (ScoreDoc sd : tds.scoreDocs) {
				...
				System.out.println(doc.get("id") + "---->"
						+ doc.get("introduction") + "[" + doc.get("title")
						+ "]-->" + doc.get("content"));
			}
		} catch (CorruptIndexException e) {
			e.printStackTrace();
	...
	public void searchByTermRange(String field, String start, String end,
		...
		try {
			IndexSearcher searcher = getSearcher();
			Query query = new TermRangeQuery(field, new BytesRef(
					start.getBytes()), new BytesRef(end.getBytes()), true, true);
			TopDocs tds = searcher.search(query, num);
			System.out.println("一共查询了:" + tds.totalHits);
			for (ScoreDoc sd : tds.scoreDocs) {
				Document doc = searcher.doc(sd.doc);
	...
	public Integer[] searchPage(String query, int pageIndex, int pageSize) {
		Integer[] ids = null;
		try {
			IndexSearcher searcher = getSearcher();
			...
			for (int i = start; i < end; i++) {
				...
				ids[flag] = Integer.parseInt(doc.get("id"));
				System.out.println(ids[flag]);
				flag++;
				System.out.println(sds[i].doc + ":" + doc.get("introduction")
						+ "-->" + doc.get("content"));
